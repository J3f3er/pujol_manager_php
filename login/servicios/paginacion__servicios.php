...
$stmt = $conexion->prepare("SELECT COUNT(*) as total_registros FROM servicios WHERE pm_usuario = :pm_usuario");
...
$stmt = $conexion->prepare("SELECT * FROM servicios WHERE pm_usuario = :pm_usuario LIMIT $limite_inicio, $registros_por_pagina");
$stmt->execute([':pm_usuario' => $id]);
