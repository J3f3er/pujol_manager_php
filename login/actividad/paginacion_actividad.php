...
$stmt = $conexion->prepare("SELECT COUNT(*) as total_registros FROM actividad WHERE pm_user = :pm_user ORDER BY pm_id DESC");
...
$stmt = $conexion->prepare("SELECT * FROM actividad WHERE pm_user = :pm_user ORDER BY pm_id DESC LIMIT $limite_inicio, $registros_por_pagina");
$stmt->execute([':pm_user' => $id]);
